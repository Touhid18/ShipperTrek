...
public class GCMController extends Application {
	...
	public void register(final Context context, final int id, final String deviceId, final String regId) {
		...
		String serverUrl = AppConstants.URL_API + "gcm";
		...
		for (int i = 1; i <= MAX_ATTEMPTS; i++) {
			...
			try {
				...
				postToServer(context, serverUrl, id, deviceId, regId);
	...
	private void postToServer(Context context, String serverUrl, int id, String deviceId, String regId)
			throws Exception {
		...
		Log.d("GCM_postToServer", "To be sent request body(JSON String): " + jStr);
		ServerResponse response = jsonParser.retrieveServerData(AppConstants.REQUEST_TYPE_POST, serverUrl, null, jStr,
				"");// TODO Token
		if (response.getStatus() == 200) {
			Log.i("GCMController", "GCM data updated to server successfully");
			if (!serverUrl.endsWith("unregister"))
	...
	public void unregister(final Context context, final String regId) {
		...
		String serverUrl = AppConstants.URL_API + "gcm/unregister";
		// Map<String, String> params = new HashMap<String, String>();
		// params.put("regId", regId);

		try {
			postToServer(context, serverUrl, 1, "", regId);
